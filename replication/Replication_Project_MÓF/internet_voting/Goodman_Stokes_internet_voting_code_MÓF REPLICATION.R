...
setwd("/Users/marcus/Documents/ASDS/ASAQM II/Replication_Project_MÓF/internet_voting")
getwd()
...
# MÓF twist.
# Testing to see if an interactive model fits the data.
mod1 <- lm(turnout ~ intvoting*log(pop_den) + log(pop) + unemploy + log(median_inc) + p_65_plus + p_uni + log(p_imm) + as.factor(csd_code) + as.factor(year), data=munis_dropped)
round(coeftest(mod1, vcov=vcovCluster(mod1, cluster=munis_dropped$csd_code))[1:108,], 3)
...
# The following lines of code were added underneath the "Table 1 column 3" section
# above. This gives us the results of an interactive model.
